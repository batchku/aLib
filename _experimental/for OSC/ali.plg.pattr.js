...
function makeppz()
{
//	outlet(0,"script new var1 newex 157 160 133 196617 pp 1 Delay 0 1023 Samples;");


	// indeces: thedatastruct[currentslot][datastructindex][k]=v[k]
	var ppcount = 0;
	post("slotnames.length: ",slotnames.length);
	post();
	post("clientnames.length: ",thedatastruct[0].length);
	post();
	//for (var i=0; i<slotnames.length;i++) {
		
		for (var j=0; j<clientnames.length;j++) {
			post("thedatastruct[0][j].length: ",thedatastruct[0][j].length);
			post();
			for (var k=0; k<thedatastruct[0][j].length;k++) {
				var y = (ppcount*20+20);
				var count = ppcount + 1;
				outlet(0, "script new "+clientnames[j]+"_"+k+" newex 800 "+y+" 133 196617 pp "+count+" "+clientnames[j]+"_"+k);
				ppcount++;
			}
		}
	//}
