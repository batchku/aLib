...
function drawlabel(n)
{
	...
	jString = n+num_offset; //added one to match pattr's preset numbering.
	//jString = n;
	jString = jString + "";
	text(jString);
	}
...
function addpoint()
{
	...
	for (j=1; j < v.length; j++) {
		//points[n][j-1]=v[j];  //going back to numbering from 1	
		//points[n][j-1]=v[j];  //going back to numbering from 1
	}
...
function placeone()
{
	place(arguments[0]-1, arguments[1], arguments[2]); //added one to match pattr numering
	if (visuals == 2) {
